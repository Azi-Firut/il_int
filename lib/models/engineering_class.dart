...
class Engineering {
    ...
    if (await createTempKeyFile()) {
      ...
      try {
       ...
       $plinkPath -i "$keyPath" -P 22 root@192.168.12.1 -hostkey "$hostKey" "mount -o remount,rw / && echo novatel >/etc/payload/receiver && exit"
        ''');
        pushUnitResponse(1,"Scanner changed successfully to $selectedScanner",updateState);

      } catch (e) {

        pushUnitResponse(2,"Failed to change: check all conditions before start",updateState);
  ...
  Future<void> changeScanner(Function updateState) async {
    if (selectedScanner != null) {
      ...
      pushUnitResponse(3,"Please select a scanner and ensure the device is connected",updateState);
    }
    updateState();
  }
}
